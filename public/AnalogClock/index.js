  ...
  function animateClocks() {
    ...
    clocks.forEach((clock) => {
      ...
      if (clock.id === "local") {
        ...
        dateText = targetTime.toLocaleDateString("en-US", {
          weekday: "short",
          month: "short",
          day: "numeric",
        });
      ...
      if (clock.hourHand)
        clock.hourHand.style.transform = `rotate(${hRotation}deg)`;
      if (clock.minuteHand)
        clock.minuteHand.style.transform = `rotate(${mRotation}deg)`;
      if (clock.secondHand)
        clock.secondHand.style.transform = `rotate(${sRotation}deg)`;
