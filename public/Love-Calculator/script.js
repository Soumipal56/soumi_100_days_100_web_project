...
function calculateLove() {
  ...
  setTimeout(() => {
    ...
    if (loveIndex <= 30) {
      ...
      msg = lowMessages[Math.floor(Math.random() * lowMessages.length)];
    } else if (loveIndex <= 60) {
      emoji = "💛";

      msg = mediumMessages[Math.floor(Math.random() * mediumMessages.length)];
    } else if (loveIndex <= 90) {
      emoji = "💕";

      msg = highMessages[Math.floor(Math.random() * highMessages.length)];
    } else {
      emoji = "❤️‍🔥";

      msg =
        soulmateMessages[Math.floor(Math.random() * soulmateMessages.length)];
    }
