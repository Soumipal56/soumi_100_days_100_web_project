...
class Particle {
  ...
  reset(init) {
    ...
    this.color =
      themes[currentTheme][
        Math.floor(Math.random() * themes[currentTheme].length)
      ];
  ...
  update(t) {
    ...
    const wave =
      Math.sin(this.x * this.freq + t * 0.015 * speed + this.phase) * this.amp;
    const wave2 =
      Math.cos(this.x * this.freq * 0.5 + t * 0.008 * speed + this.phase) *
      (this.amp * 0.3);
