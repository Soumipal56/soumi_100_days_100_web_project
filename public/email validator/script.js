...
function validateFormat(email) {
  ...
  if (local.length === 0) {
    errors.push(
      "The part before @ is empty — add your username (e.g. yourname@gmail.com)",
    );
  } else {
    if (local.startsWith("."))
    ...
    if (/[^a-zA-Z0-9._%+\-]/.test(local))
      errors.push(
        "Local part contains an invalid character — only letters, numbers, and . _ % + - are allowed",
      );
  }
  ...
  if (domain.length === 0) {
    ...
    if (!domain.includes("."))
      errors.push(
        "Domain is missing a dot — it should look like gmail.com or yahoo.in",
      );
    ...
    if (/\s/.test(domain)) errors.push("Spaces are not allowed in the domain");
    if (/[^a-zA-Z0-9.\-]/.test(domain))
      errors.push(
        "Domain contains an invalid character — only letters, numbers, dots, and hyphens are allowed",
      );
    ...
    if (tld && tld.length < 2)
      errors.push(
        "Top-level domain (." +
          tld +
          ") is too short — use .com, .in, .org etc.",
      );
  }
...
function showFormatErrors(errors) {
  formatErrorList.innerHTML = errors
    .map(function (e) {
      return "<li>" + e + "</li>";
    })
    .join("");
...
function handleValidate() {
  ...
  if (isDisposable) {
    ...
    reason =
      "This is a temporary/throwaway email address and is not accepted in most services";
    ...
    reason =
      'The domain "' +
      domain +
      '" does not exist or is not a recognised email provider. Did you mean ' +
      fuzzy.suggestion +
      "?";
    statusClass = "status-bad";
  } else if (isKnown) {
    ...
    reason =
      'The domain "' +
      domain +
      '" is not a known email provider. It may be valid (company/org domain) but could not be verified';
  ...
  var html = "";
  html +=
    '<div class="result-item"><strong>Email:</strong><span>' +
    email +
    "</span></div>";
  html +=
    '<div class="result-item"><strong>Status:</strong><span class="' +
    statusClass +
    '">' +
    status +
    "</span></div>";
  html +=
    '<div class="result-item"><strong>Domain:</strong><span>' +
    domain +
    "</span></div>";
  html +=
    '<div class="result-item"><strong>Format Valid:</strong><span>✅ Yes</span></div>';
  html +=
    '<div class="result-item"><strong>Free Email:</strong><span>' +
    (isFree ? "Yes" : "No") +
    "</span></div>";
  html +=
    '<div class="result-item"><strong>Disposable:</strong><span>' +
    (isDisposable ? "⚠️ Yes — temporary email" : "✅ No") +
    "</span></div>";
  html +=
    '<div class="result-item"><strong>Domain Known:</strong><span>' +
    (isKnown
      ? "✅ Yes"
      : isDomainTypo
        ? "❌ Looks like a typo"
        : "⚠️ Unknown (may be org domain)") +
    "</span></div>";
  html +=
    '<div class="result-item"><strong>Reason:</strong><span>' +
    reason +
    "</span></div>";
