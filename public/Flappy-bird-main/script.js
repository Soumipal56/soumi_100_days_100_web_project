...
function spawnParticles(x, y) {
  for (let i = 0; i < 7; i++) {
    ...
    particles.push({
      ...
      maxLife: 30,
      r: 2 + Math.random() * 2,
      color: ["#ffe082", "#fff176", "#ffffff", "#ffcc02"][
...
function updatePipes(dt) {
  for (let i = pipes.length - 1; i >= 0; i--) {
    ...
    if (
      inX &&
      (bird.y - BIRD_R < p.topH ||
        bird.y + BIRD_R > canvas.height - GROUND_H - p.botH)
    ) {
      if (!gameOver) {
        playDeath();
        deathShake = SHAKE_FRAMES;
...
function getMedal(s) {
  ...
  if (s >= 20)
    return { label: "🥇 GOLD", color: "#ffe082", glow: "rgba(255,220,50,0.6)" };
  if (s >= 10)
    return {
      label: "🥈 SILVER",
      color: "#cfd8dc",
...
function drawGameOver() {
  ...
  ctx.beginPath();
  ctx.moveTo(canvas.width / 2 - 210, 232);
  ctx.lineTo(canvas.width / 2 + 210, 232);
  ctx.stroke();
